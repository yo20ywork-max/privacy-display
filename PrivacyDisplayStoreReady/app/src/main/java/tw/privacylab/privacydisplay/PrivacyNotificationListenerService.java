...
public class PrivacyNotificationListenerService extends NotificationListenerService {
    ...
    public void onNotificationPosted(StatusBarNotification sbn) {
        ...
        if (!prefs.getBoolean(SettingsStore.KEY_CONSENT_GRANTED, false)) return;
        if (!prefs.getBoolean(SettingsStore.KEY_NOTIFICATION_MASK_ENABLED, false)) return;
    ...
    private void sendGenericHiddenNotification(String packageName) {
        ...
        Notification notification = builder
                ...
                .setContentText(getAppLabel(packageName) + " 的通知已由 Privacy Display 保護。")
    ...
    private void createChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            NotificationChannel channel = new NotificationChannel(
                    CHANNEL_ID,
                    "Privacy Display 通知防窺",
                    NotificationManager.IMPORTANCE_DEFAULT
            );
            channel.setDescription("顯示已被保護的指定 App 通知提醒。");
            NotificationManager manager = (NotificationManager) getSystemService(NOTIFICATION_SERVICE);
